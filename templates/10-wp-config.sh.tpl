...
if [[ -f "/docker-entrypoint-initwp.d/wp-salts.txt" ]]; then
  cat /docker-entrypoint-initwp.d/wp-salts.txt >> "$WP_CONFIG"
fi

if ! grep -q "WP_HOME" "$WP_CONFIG"; then
  echo "define('WP_HOME', getenv('WP_HOME'));" >> "$WP_CONFIG"
  echo "define('WP_SITEURL', getenv('WP_SITEURL'));" >> "$WP_CONFIG"
fi
...
if ! grep -q "HTTP_X_FORWARDED_PROTO" "$WP_CONFIG"; then
  {
    echo ""
    echo "// Fix for HTTPS behind reverse proxy (CloudFront)"
